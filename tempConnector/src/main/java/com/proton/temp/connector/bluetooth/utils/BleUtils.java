...
public class BleUtils {
     ...
     * 计算每个缓存的时间
     */
    public static List<TempDataBean> getTempTime(List<TempDataBean> cacheTemps, long lastReceiveTempTime) {
        long lastTempTime = System.currentTimeMillis();
        if (System.currentTimeMillis() - lastReceiveTempTime < 1000) {
            //连续的数据
            lastTempTime = lastTempTime + 10000 * count;
            count++;
        } else {
            count = 1;
        }
    ...
    public static List<TempDataBean> getTempTime(List<TempDataBean> cacheTemps) {
        return getTempTime(cacheTemps, System.currentTimeMillis());
    }
