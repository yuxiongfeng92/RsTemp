...
import com.wms.logger.BuildConfig;
...
public class AlgorithmManager {
    ...
    private static int algorithmType = 2;
     ...
     * @param time         当前温度时间
     * @param sample       采样率
     * @param connectStyle 连接方式 0 蓝牙 1广播
     */
    public void doAlgorithm1_5(float currentTemp, long time, int sample, int connectStyle) {
        if (algorithmStatus == 2) {
            algorithmStatus = 0;
        }
        tempResult = algorithm.getTemp(currentTemp, time, getFlag(), algorithmStatus, isPill ? 1 : 0, sample, connectStyle, algorithmType);
        //测量状态改为正在测量
        algorithmStatus = 1;
        if (tempResult != null) {
    ...
    public AlgorithmManager setAlgorithmStatus(int status) {
        ...
        algorithm.getTemp(-1, -1, getFlag(), status, 0, 1, -1, algorithmType);
        return this;
    }
