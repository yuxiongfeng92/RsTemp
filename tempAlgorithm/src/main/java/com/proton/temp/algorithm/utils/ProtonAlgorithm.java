...
public class ProtonAlgorithm implements IAlgorithm {
    ...
    @Override
    public TempResult getTemp(float temp, long time, int flag, int state, int isPill, int sample, int connectStyle, int algorithmType) {
        return AlgorithmHelper.getTemp(temp, time, flag, state, isPill, sample, connectStyle, algorithmType);
    }
}
