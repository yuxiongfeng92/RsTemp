...
public class StatusBarUtil {
     ...
     *
     * @param activity   需要设置的activity
     * @param background 状态栏drawable
     * @return 状态栏矩形条
     */
    private static View createStatusBarDrawable(Activity activity, @DrawableRes int background) {
        // 绘制一个和状态栏一样高的矩形
        View statusBarView = new View(activity);
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight());
        statusBarView.setLayoutParams(params);
        statusBarView.setBackground(ContextCompat.getDrawable(App.get(), background));
        statusBarView.setId(FAKE_STATUS_BAR_VIEW_ID);
        return statusBarView;
    }


    public static void setStatusBarDrawable(Activity activity, @DrawableRes int background) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        setTransparent(activity);
        ...
        if (fakeTranslucentView != null) {
            ...
            contentView.addView(createStatusBarDrawable(activity, background));
        }
    }
    ...
    public static int getStatusBarHeight() {
        ...
        return App.get().getResources().getDimensionPixelSize(resourceId);
    }
